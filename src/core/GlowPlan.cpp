...
int BaseScaleForQuality(float sigma, Quality quality, int max_base_scale) {
    ...
    int scale = 1;
    while (scale * 2 <= max_base_scale && static_cast<float>(scale * 2) <= target) scale *= 2;
    return scale;
}
...
GlowPlan MakeGlowPlan(float sigma, Quality quality, int max_base_scale) {
    GlowPlan plan;
    plan.level_sigma = LevelSigmaForQuality(quality);
    plan.base_scale = BaseScaleForQuality(sigma, quality, std::max(1, max_base_scale));
